...
export class DatabaseAPIRequests {
  ...
  async discoverSentinelDatabaseApi(
    ...
    const resourcePath = ResourcePath.RedisSentinel + ResourcePath.Databases
    const response = await CommonAPIRequests.sendPostRequest(
      resourcePath,
      requestBody,
    )
  ...
  async getDatabaseIdByName(databaseName?: string): Promise<string> {
    ...
    const response = await Common.asyncFilter(
      allDataBases,
      async (item: databaseParameters) => {
        await Common.doAsyncStuff()
  ...
  async getDatabaseByConnectionType(connectionType?: string): Promise<string> {
    ...
    response = await Common.asyncFilter(
      allDataBases,
      async (item: databaseParameters) => {
        await Common.doAsyncStuff()
  ...
  async getClusterNodesApi(
    ...
    const response = await CommonAPIRequests.sendGetRequest(resourcePath)
