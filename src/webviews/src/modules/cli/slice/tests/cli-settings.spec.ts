...
import { concatToOutput, setCliDbIndex } from 'uiSrc/modules/cli/slice/cli-output'
import {
  cliTexts,
...
describe('cliSettings slice', () => {
  beforeAll(() => {
    mathRandom = vi.spyOn(Math, 'random').mockImplementation(() => random)
  })
  ...
  describe('thunks', () => {
    // skipped until DB connection will be implemented
    it.skip('call both createCliClientAction and processCliClientSuccess when fetch is successed', async () => {
      // Arrange
      const data = { uuid: '70b95d32-c19d-4311-bb24-e684af12cf15' }
    ...
    // skipped until DB connection will be implemented
    it.skip('call both createCliClientAction and processCliClientFailure when fetch is fail', async () => {
      // Arrange
